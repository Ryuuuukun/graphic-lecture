    ...
    scene->load_models({
        {
            ...
            .transform = fin::rotate_y(0.0f),
            .material  = std::make_shared<Material>(color3f(0.7f))
        },
        {
            .filename  = "../resources/torus.obj",
            .transform = fin::rotate_x(std::numbers::pi / 2) | fin::translate(-1.0f, 0.0f, 0.0f),
            .material  = std::make_shared<Material>(color3f(0.2f, 0.6f, 0.9f))
        }
    });

    auto camera = std::make_shared<Camera>(900.0f, vec3f{ 0, -5, 5 }, vec3f{ 0, 0, 0 });

    // auto algorithm = std::make_shared<SimpleTracing>(color3f{ 32.0f }, color3f{ 255.0f });
    auto algorithm = std::make_shared<ShadowLightTracing>(
        light3f{ .origin = { -5.0f,  0.0f, -2.0f }, .color = { 1.0f  }, .intensity = 255.0f }, // direct
